...
class RollMultiRollNeuralNet(RollMultiNeuralNet):
    ...
    """
    def __call__(
            self, y, X, NN, weights=[], start=0, end=1e6, x_axis=None, 
            reset_nn=True
        ):
        ...
        :reset_nn: bool or int
            If int reset one neural network each `reset_nn` periods.

        Returns
        -------
        :self: RollMultiRollNeuralNet (Object)
    ...
    def run(
            self, y, X, NN, weights=[], plot_loss=True, plot_perf=True, 
