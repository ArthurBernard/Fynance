...
# @Last modified time: 2019-10-16 11:13:30
...
def sma(X, k, axis=0, dtype=None):
    r""" Compute simple moving average(s) for `k` lags for each `X`' series.
    ...
    array([[ 60.,  60.],
           ...
           [120., 120.],
           [120., 120.]])
    >>> sma(X, k=3, dtype=np.float64, axis=1)
    array([[ 60.,  60.],
           [100., 100.],
           [ 80.,  80.],
           [120., 120.],
    ...
    """
    return _sma(X, k, dtype=dtype, axis=axis)


def _sma(X, k, axis=0, dtype=None):
    if len(X.shape) == 2:

        return np.asarray(sma_cy_2d(X, k))

    return np.asarray(sma_cy_1d(X, k))


@WrapperArray('dtype', 'axis', 'lags')
def wma(X, k, axis=0, dtype=None):
    r""" Compute weighted moving average(s) for `k` lags for each `X`' series.

    .. math::
        wma_t = \frac{2}{k (k-1)} \sum^{k-1}_{i=0} (k-i) \times X_{t-i}
    ...
    """
    return _wma(X, k, axis=axis, dtype=dtype)


def _wma(X, k, axis=0, dtype=None):
    if len(X.shape) == 2:

        return np.asarray(wma_cy_2d(X, k))

    return np.asarray(wma_cy_1d(X, k))


@WrapperArray('dtype', 'axis')
def ema(X, alpha=0.94, k=None, axis=0, dtype=None):
    r""" Compute exponential moving average(s) for each `X`' series.

    .. math::
        ema_t = \alpha \times ema_{t-1} + (1-\alpha) \times X_t
    ...
    k : int, optional
        Number of lags used for computation, must be positive. Default is None.
    axis : {0, 1}, optional
        Axis along wich the computation is done. Default is 0.
    dtype : np.dtype, optional
        The type of the output array.  If `dtype` is not given, infer the data
    ...
    Notes
    -----
    If `k` lags is specified :math:`\alpha` is overwritten by
    :math:`\alpha = 1 - \frac{2}{1 + k}`
...
def _ema(X, alpha=0.94, axis=0, dtype=None):
    if len(X.shape) == 2:
...
@WrapperArray('dtype', 'axis', 'lags')
def smstd(X, k, axis=0, dtype=None):
    r""" Compute simple moving standard deviation(s) for each `X`' series'.

    .. math::
        smstd_t = \sqrt{\frac{1}{k} \sum^{k-1}_{i=0} (X_{t-i} - sma_t)^2}
    ...
    --------
    >>> X = np.array([60, 100, 80, 120, 160, 80])
    >>> smstd(X, 3, dtype=np.float64)
    array([ 0.        , 20.        , 16.32993162, 16.32993162, 32.65986324,
           32.65986324])
    >>> smstd(X.reshape([6, 1]), 3, dtype=np.float64).flatten()
    array([ 0.        , 20.        , 16.32993162, 16.32993162, 32.65986324,
           32.65986324])
    ...
    """
    return _smstd(X, k, axis=axis, dtype=dtype)


def _smstd(X, k, axis=0, dtype=None):
    if len(X.shape) == 2:

        return np.asarray(smstd_cy_2d(X, k))

    return np.asarray(smstd_cy_1d(X, k))


@WrapperArray('dtype', 'axis', 'lags')
def wmstd(X, k, axis=0, dtype=None):
    ...
    --------
    >>> X = np.array([60, 100, 80, 120, 160, 80])
    >>> wmstd(X, 3, dtype=np.float64)
    array([ 0.        , 18.85618083, 13.74368542, 17.95054936, 29.8142397 ,
           35.90109871])
    >>> wmstd(X.reshape([6, 1]), 3, dtype=np.float64).flatten()
    array([ 0.        , 18.85618083, 13.74368542, 17.95054936, 29.8142397 ,
           35.90109871])
    ...
    """
    return _wmstd(X, k, axis=axis, dtype=dtype)


def _wmstd(X, k, axis=0, dtype=None):
    if len(X.shape) == 2:

        return np.asarray(wmstd_cy_2d(X, k))

    return np.asarray(wmstd_cy_1d(X, k))


@WrapperArray('dtype', 'axis')
def emstd(X, alpha=0.94, k=None, axis=0, dtype=None):
    r""" Compute exponential moving standard deviation(s) for each `X`' series.

    .. math::
        emstd_t = \sqrt{\alpha\times emstd_{t-1}^2+(1-\alpha)\times X_t^2}
    ...
    --------
    >>> X = np.array([60, 100, 80, 120, 160, 80])
    >>> emstd(X, k=3, dtype=np.float64)
    array([ 0.        , 14.14213562, 10.        , 15.8113883 , 23.97915762,
           24.49489743])
    >>> emstd(X.reshape([6, 1]), k=3, dtype=np.float64).flatten()
    array([ 0.        , 14.14213562, 10.        , 15.8113883 , 23.97915762,
           24.49489743])
...
def _emstd(X, alpha=0.94, axis=0, dtype=None):
    if len(X.shape) == 2:
